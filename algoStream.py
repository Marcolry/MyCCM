...
c00MCom = np.array(['00MCom', st.secrets["aK00MCom"], st.secrets["aS00MCom"], 0.3, 0, 0])
...
allUsers = np.array([c00TFio, c00Xm33, c00MCom, c01Vitor, c02Joao, c03Rayan])
...
session_auth_00MCom = usdt_perpetual.HTTP(endpoint="https://api.bybit.com",api_key=st.secrets["aK00MCom"],api_secret=st.secrets["aS00MCom"])
...
authUSDT_00MCom = session_auth_00MCom.get_wallet_balance()
...
posUSDT_00MCom = session_auth_00MCom.my_position()
...
DEPOSIT_00TFio = EQUITY_00TFio - NRZ_00TFio - PERF_00TFio -2000
PNL100_00TFio = (round(EQUITY_00TFio/DEPOSIT_00TFio,4)-1)*100

TAXABLE_00TFio = NRZ_00TFio + PERF_00TFio - 1000
...
ALL_00MCom = list(range(0, len(posUSDT_00MCom['result']))) #print(ALL)
a_00MCom = arr.array('i', ALL_00MCom)

######################################################################################### DATA

EQUITY_00MCom = authUSDT_00MCom['result']['USDT']['equity']
NRZ_00MCom = authUSDT_00MCom['result']['USDT']['unrealised_pnl']
PERF_00MCom = authUSDT_00MCom['result']['USDT']['cum_realised_pnl']
AVAILABLE_00MCom = authUSDT_00MCom['result']['USDT']['available_balance']
INPLAY_00MCom = (EQUITY_00MCom - AVAILABLE_00MCom)/EQUITY_00MCom


DEPOSIT_00MCom = EQUITY_00MCom - NRZ_00MCom - PERF_00MCom
PNL100_00MCom = (round(EQUITY_00MCom/DEPOSIT_00MCom,4)-1)*100

TAXABLE_00MCom = NRZ_00MCom + PERF_00MCom

if TAXABLE_00MCom > 0:
    COMMISSION_00MCom = TAXABLE_00MCom * TXCOM_00MCom
if TAXABLE_00MCom <= 0:
    COMMISSION_00MCom = 0
...
for x in a_00MCom:
   oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
   if oneTrade:
       nbTrade_00MCom += 1

for x in a_00MCom:
   oneLong = posUSDT_00MCom['result'][x]['data']["side"] == "Buy" and posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
   if oneLong:
       nbLong_00MCom += 1

for x in a_00MCom:
   oneShort = posUSDT_00MCom['result'][x]['data']["side"] == "Sell" and posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
   if oneShort:
       nbShort_00MCom += 1
...
for i in range(13):
    if i == 0:
        with col1:
            ...
            st.info('💁🏾‍♀️'' 00Xm33')
            st.info('💁🏾‍♀️'' 00MCom')
            st.info("💰"' : ' + str(round((EQUITY_00TFio + EQUITY_00Xm33 + EQUITY_00MCom)/float(btc), 4)) + ' BTC')
    if i == 1:
        with col2:
            st.info("💰 Equity 💰")
            st.info("💰"' : ' + str(round(EQUITY_00TFio,2)))
            st.info("💰"' : ' + str(round(EQUITY_00Xm33,2)))
            st.info("💰"' : ' + str(round(EQUITY_00MCom,2)))
            st.info("💰"' : ' + str(round(EQUITY_00TFio + EQUITY_00Xm33 + EQUITY_00MCom, 2)))
    if i == 2:
        with col3:
            st.info("⬇️ Deposit ⬇️")
            st.info("⬇️"' : ' + str(round(DEPOSIT_00TFio,2)))
            st.info("⬇️"' : ' + str(round(DEPOSIT_00Xm33,2)))
            st.info("⬇️"' : ' + str(round(DEPOSIT_00MCom,2)))
            st.info("⬇️"' : ' + str(round(DEPOSIT_00TFio + DEPOSIT_00Xm33 + DEPOSIT_00MCom, 1)))
    if i == 3:
        with col4:
            st.info("💸 Bénéfice 💸")
            st.info("💸"' : ' + str(round(TAXABLE_00TFio,2)))
            st.info("💸"' : ' + str(round(TAXABLE_00Xm33,2)))
            st.info("💸"' : ' + str(round(TAXABLE_00MCom,2)))
            st.info("💸"' : ' + str(round(TAXABLE_00TFio + TAXABLE_00Xm33 + TAXABLE_00MCom, 2)))
    if i == 4:
        with col5:
            st.info("💸 Realized 💸")
            st.info("💸"' : ' + str(round(PERF_00TFio,2)))
            st.info("💸"' : ' + str(round(PERF_00Xm33,2)))
            st.info("💸"' : ' + str(round(PERF_00MCom,2)))
            st.info("💸"' : ' + str(round(PERF_00TFio + PERF_00Xm33 + PERF_00MCom, 2)))
    if i == 5:
        with col6:
            st.info("💸 Unrealized 💸")
            st.info("💸"' : ' + str(round(NRZ_00TFio,2)))
            st.info("💸"' : ' + str(round(NRZ_00Xm33,2)))
            st.info("💸"' : ' + str(round(NRZ_00MCom,2)))
            st.info("💸"' : ' + str(round(NRZ_00TFio + NRZ_00Xm33 + NRZ_00MCom, 2)))
    if i == 6:
        with col7:
            st.info("💀 In Play 💀")
            st.info("💀"' : ' + str(round(INPLAY_00TFio*100,2)) +'%')
            st.info("💀"' : ' + str(round(INPLAY_00Xm33*100,2)) +'%')
            st.info("💀"' : ' + str(round(INPLAY_00MCom*100,2)) +'%')
            st.info("💀"' : ' + str(round((EQUITY_00TFio + EQUITY_00Xm33 + EQUITY_00MCom - AVAILABLE_00TFio - AVAILABLE_00Xm33 - AVAILABLE_00MCom) * 100 / (EQUITY_00TFio + EQUITY_00Xm33 + EQUITY_00MCom), 2)) + '%')
    if i == 7:
        with col8:
            st.info("🫶 L/S 🫶")
            if nbTrade_00TFio == 0:
            ...
            if nbTrade_00MCom == 0:
                ...
                st.info("🫶"' : ' + str(nbLong_00MCom) + ' / ' + str(nbShort_00MCom) + ' (' + str(round((nbLong_00MCom/nbTrade_00MCom)*100)) + '/' + str(round((nbShort_00MCom/nbTrade_00MCom)*100)) + '%)')
            st.info("🫶 Long/Short 🫶")
    if i == 8:
        with col9:
            st.info("#️⃣ Nb. T #️⃣")
            ...
            st.info("#️⃣"' : ' + str(nbTrade_00MCom))
            st.info("#️⃣"' : ' + str(nbTrade_00TFio + nbTrade_00Xm33 + nbTrade_00MCom))
    if i == 9:
        with col10:
            st.info('〽️ Perf. 〽️')
            st.info('〽️'' : ' + str(round(PNL100_00TFio,3)) +'%')
            st.info('〽️'' : ' + str(round(PNL100_00Xm33,3)) +'%')
            st.info('〽️'' : ' + str(round(PNL100_00MCom,3)) +'%')
            st.info('〽️'' : ' + str(round((PNL100_00TFio * DEPOSIT_00TFio + PNL100_00Xm33 * DEPOSIT_00Xm33 + PNL100_00MCom + DEPOSIT_00MCom) / (DEPOSIT_00TFio + DEPOSIT_00Xm33 + DEPOSIT_00MCom), 2)) + '%')
    if i == 10:
        with col11:
            st.info('✂️ HWM ✂️')
            st.info('✂️'' : ' + str(round(HWM_00TFio)))
            st.info('✂️'' : ' + str(round(HWM_00Xm33)))
            st.info('✂️'' : ' + str(round(HWM_00MCom)))
            st.info('✂️'' : ' + str(round(HWM_00TFio+HWM_00Xm33+HWM_00MCom)))
    if i == 11:
        with col12:
            st.info('✂️ Com. ✂️')
            st.info('✂️'' : ' + str(round(COMMISSION_00TFio-HWM_00TFio,2)))
            st.info('✂️'' : ' + str(round(COMMISSION_00Xm33-HWM_00Xm33,2)))
            st.info('✂️'' : ' + str(round(COMMISSION_00MCom-HWM_00MCom,2)))
            st.info('✂️'' : ' + str(round(COMMISSION_00TFio + COMMISSION_00Xm33 + COMMISSION_00MCom - HWM_00TFio - HWM_00Xm33 - HWM_00MCom, 2)))
    if i == 12:
        with col13:
            st.info('✂️ Retro C. ✂️')
            ...
            if RETRO_00MCom == 0 or COMMISSION_00MCom-HWM_00MCom <= 0:
                st.info("🫶"' : N/A')
            else:
                st.info('✂️'' : ' + str(round((COMMISSION_00MCom-HWM_00MCom)*RETRO_00MCom,2)))
            st.info('✂️'' : ' + str(round((COMMISSION_00TFio - HWM_00TFio) * RETRO_00TFio + (COMMISSION_00Xm33 - HWM_00Xm33) * RETRO_00Xm33 + (COMMISSION_00MCom - HWM_00MCom) * RETRO_00MCom, 2)))
    ...
    if i == 1:
        with col1_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    st.info(str(posUSDT_00MCom['result'][x]['data']['symbol']))
    if i == 2:
        with col2_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    st.info(str(posUSDT_00MCom['result'][x]['data']['side']))
    if i == 3:
        with col3_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    st.info(str(posUSDT_00MCom['result'][x]['data']['size']))
    if i == 4:
        with col4_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    if posUSDT_00MCom['result'][x]['data']['unrealised_pnl'] >= 0:
                        st.success(str(round(posUSDT_00MCom['result'][x]['data']['unrealised_pnl'],2)))
                    else:
                        st.error(str(round(posUSDT_00MCom['result'][x]['data']['unrealised_pnl'],2)))
    if i == 5:
        with col5_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    if posUSDT_00MCom['result'][x]['data']['unrealised_pnl'] >= 0:
                        st.success(str(round((posUSDT_00MCom['result'][x]['data']['unrealised_pnl']/(posUSDT_00MCom['result'][x]['data']['entry_price']*posUSDT_00MCom['result'][x]['data']['size']))*100,2))+ '%')
                    else:
                        st.error(str(round((posUSDT_00MCom['result'][x]['data']['unrealised_pnl']/(posUSDT_00MCom['result'][x]['data']['entry_price']*posUSDT_00MCom['result'][x]['data']['size']))*100,2))+ '%')
    if i == 6:
        with col6_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    st.info(str(posUSDT_00MCom['result'][x]['data']['cum_realised_pnl']))
    if i == 7:
        with col7_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    st.info(str(posUSDT_00MCom['result'][x]['data']['entry_price']))
    if i == 8:
        with col8_01:
            for x in a_00MCom:
                oneTrade = posUSDT_00MCom['result'][x]['data']["entry_price"] != 0
                if oneTrade:
                    st.info(str(round(posUSDT_00MCom['result'][x]['data']['entry_price']*posUSDT_00MCom['result'][x]['data']['size'],2)))
